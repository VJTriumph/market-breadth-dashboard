...
def build_dashboard():
    ...
    for _, row in stocks_df.iterrows():
        ...
        last_date_str = sig.pop("last_date", None)
        if last_date_str:
    ...
    df["DataWarning"]  = df.apply(
        lambda r: "WARNING" if r["StocksOK"] < r["StocksTotal"] else "OK", axis=1
    )
    ...
    if stale_tickers:
        print("  STALE STOCKS:", [f"{t['stock']}({t['last_date']})" for t in stale_tickers])
